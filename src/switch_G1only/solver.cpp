...
void Solver::solve_endemic_eq()
{
    ...
    for (unsigned long int ecol_time = 0; ecol_time < par.max_time_ecology;
            ++ecol_time)
    {
...
//        std::cout << N << " " << ecol_time << " " << dSdt() << " " << dG1dt() << " " << dG2dt() << " " << dG1G2dt() << " ";
...
void Solver::eigenvectors()
{
    ...
    m(S_idx,S_idx) = b(S_idx) * (1.0 - par.kappa * N) -
        (par.d[S_idx] + par.psi[G1_idx] + par.psi[G2_idx]);
    ...
    // row 2:
    m(G1_idx, S_idx) = par.psi[G1_idx];

    m(G1_idx, G1_idx) = b(G1_idx) * (1.0 - par.kappa * N) - 
        (par.d[G1_idx] + par.gamma[G1_idx] + par.psi[G2_idx]);

    m(G1_idx, G2_idx) = (1.0 - par.sigma) * (1.0 - pi) * par.psi[G1_idx];
    ...
    m(G2_idx, G1_idx) = (1.0 - par.sigma) * par.psi[G2_idx];

    m(G2_idx, G2_idx) = b(G2_idx) * (1.0 - par.kappa * N) -
        (par.d[G2_idx] + par.gamma[G2_idx] + (1.0 - pi) * par.psi[G1_idx]);
...
double Solver::dG1dt()
{
    return(b(G1_idx) * (1.0 - par.kappa * N) * popsizes[G1_idx]
            + par.psi[G1_idx] * popsizes[S_idx]
            - (par.d[G1_idx] + par.gamma[G1_idx] + 
                par.sigma * par.psi[G2_idx] + (1.0 - par.sigma) * par.psi[G2_idx]) * popsizes[G1_idx]
            + par.gamma[G2_idx] * popsizes[G1G2_idx]
            + (1.0 - par.sigma) * (1.0 - pi) * par.psi[G1_idx] * popsizes[G2_idx] //infection of a G2 by a G1 and as we have no superinfection hence subsequent switching to G1
            );
...
double Solver::dG2dt()
{
    return(b(G2_idx) * (1.0 - par.kappa * N) * popsizes[G2_idx]
            ...
            - (par.d[G2_idx] + par.gamma[G2_idx] + 
                par.sigma * (1.0 - pi) * par.psi[G1_idx] +
                (1.0 - par.sigma) * (1.0 - pi) * par.psi[G1_idx]
                ) * popsizes[G2_idx]
            + par.gamma[G1_idx] * popsizes[G1G2_idx]
            + (1.0 - par.sigma) * par.psi[G2_idx] * popsizes[G1_idx] // infectin of G1 by G2 - no superinfection
            );
} // end dG2dt
