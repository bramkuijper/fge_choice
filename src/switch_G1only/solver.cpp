...
double Solver::selgrad_pi()
{
    ...
    a[G2_idx][S_idx] = 0.0;
    a[G2_idx][G1_idx] = 0.0;
    a[G2_idx][G2_idx] = dbdpi(G2_idx) * (1.0 - par.kappa * N) + par.sigma * par.psi[G1_idx] + (1.0 - par.sigma) * par.psi[G1_idx];
...
void Solver::eigenvectors()
{
    ...
    m(G1_idx, G2_idx) = (1.0 - par.sigma) * (1.0 - pi) * par.psi[G1];
    ...
    m(G2_idx, G1_idx) = (1.0 - par.sigma) * par.psi[G2];
