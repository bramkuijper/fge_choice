...
file_names <- c("output_4","output_no_antibiotics_4")
...
for (file_idx in 1:length(file_names))
{
    ...
    time_series_data <- time_series_data %>% mutate(
        ...
        ,SensitiveM13d= (IPG1) / (IPG1 + IPG2 + SP)
...
all_time_series_data_l <- all_time_series_data_l %>% mutate(
            m13=ifelse(regexpr(pattern="M13s",text=Type)!=-1,"M13s","M13d")
            ,ctype=ifelse(regexpr(pattern="Immune",text=Type)!=-1,"Immune","Susceptible")
            )
...
ggsave(file="time_course_multipanel.pdf")
